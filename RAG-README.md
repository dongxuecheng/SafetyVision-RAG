...
功能：提供知识问答UI对话界面(chainlit)和文档管理接口（复用）即可，根据用户问题，从知识库中搜索相关文档并生成答案
部署：rag知识问答系统使用Docker进行部署，模型接口复用已有的docker的vllm，qdrant也复用docker中已有的qdrant
要求：代码简洁、优雅、易懂、易维护、项目架构清晰、最新、最佳实践
其中，RAG知识问答系统与SafetyVison-RAG系统相当于都是一个三层架构，底层架构复用共同的模型与向量库（二者是不同的collection），中层架构复用文档管理接口，不同的是各自的顶层逻辑