...
async def vlm_chain(image_b64: str) -> str:
    ...
    messages = [HumanMessage(content=[
        {"type": "text", "text": "你是安全专家。分析图片并简洁列出3-5个最重要的安全隐患，每条用一行描述。"},
...
async def upload_documents(
    ...
    for file in files:
        if not file.filename.endswith('.pdf'):
            details.append(DocumentDetail(filename=file.filename, status="failed", message="Not a PDF"))
            continue
        
        content = await file.read()
        if len(content) > Config.MAX_FILE_SIZE:
            details.append(DocumentDetail(filename=file.filename, status="failed", message="File too large"))
            continue
        
        # Check if exists
        result = qdrant_client.scroll(
            collection_name=Config.COLLECTION,
            scroll_filter={"must": [{"key": "metadata.filename", "match": {"value": file.filename}}]},
            limit=1,
            with_vectors=False
        )
        
        if len(result[0]) > 0 and skip_existing:
            details.append(DocumentDetail(filename=file.filename, status="skipped", message="Already exists"))
            continue
        ...
        try:
            ...
            for doc in docs:
                doc.metadata.update({"filename": file.filename, "upload_time": datetime.now().isoformat()})
...
async def delete_documents(filenames: List[str] = Query(...)):
    ...
    for filename in filenames:
        result = qdrant_client.scroll(
            collection_name=Config.COLLECTION,
            scroll_filter={"must": [{"key": "metadata.filename", "match": {"value": filename}}]},
            limit=10000,
            with_vectors=False
        )
        ...
        qdrant_client.delete(
            collection_name=Config.COLLECTION,
            points_selector=Filter(must=[FieldCondition(key="metadata.filename", match=MatchValue(value=filename))])
        )
        
        results.append({"filename": filename, "status": "deleted", "chunks_removed": len(result[0])})
...
async def list_documents():
    ...
    return [{"filename": name, "chunks_count": count} for name, count in all_docs.items()]
